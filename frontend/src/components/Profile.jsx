...
export  const Profile = () => {
  ...
  useEffect(()=>{
   ...
   const getEvents=async()=>
   {
    ...
    const ans=await axios.post("http://localhost:5000/GeteventRegister",{id:id}); 
   ...
   const getProfile=async()=>
   {
    const id=localStorage.getItem("user");
    console.log(id +" "+typeof(id));
    const ans=await axios.post("http://localhost:5000/user",{id:id}); // fetched the data from the backend
    //console.log(ans.data);
